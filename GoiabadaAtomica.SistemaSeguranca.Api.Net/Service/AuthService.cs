...
namespace GoiabadaAtomica.ApiAutenticacao.Net.Service
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<UserDTO> RegisterAsync(RegistrationRequestDTO registrationRequestDTO)
        {
            ...
            var conflictMsg = await _userRepository.CheckConflictAsync(registrationRequestDTO.Username, registrationRequestDTO.Email);
            if (conflictMsg is not null)
            {
                _logger.LogWarning("Tentativa de registro falhou: {msg}", conflictMsg);
                throw new InvalidOperationException(conflictMsg);
            }
            ...
            var registeredUser = _userRepository.RegisterUserAsync(userEntity);
            _logger.LogDebug("Usuario persistido com sucesso, ID: [{UserId}]", registeredUser.Result.Id);
            var userDTO = await _userRepository.GetUserByIdAsync(registeredUser.Result.Id);
            if (userDTO is null)
            {
        ...
        public async Task<LoginResponseDTO?> LoginAsync(LoginRequestDTO loginRequestDTO)
        {
            ...
            var passwordHash = BCrypt.Net.BCrypt.HashPassword(loginRequestDTO.Password);
            _logger.LogDebug("Buscando e validando usuario no banco de dados!");
            var loginDTO = await _userRepository.GetUserByLoginAsync(loginRequestDTO.Username);
            ...
            if (!BCrypt.Net.BCrypt.Verify(loginRequestDTO.Password, loginDTO.PasswordHash))
            {
                return null;
            }
            return new LoginResponseDTO
            {
                Token = GenerateJwtToken(loginDTO)
            };
        }
        ...
        private string GenerateJwtToken(LoginDTO loginDTO)
        {
            ...
            if (loginDTO.Roles is not null && loginDTO.Roles.Any())
            {
                foreach (var userRole in loginDTO.Roles)
                {
